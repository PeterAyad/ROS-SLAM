...
#     map_msg.info.width = int(DIM)
#     map_msg.info.height = int(DIM)
#     map_msg.info.origin.position.x = RES * -DIM/2
#     map_msg.info.origin.position.y = RES * -DIM/2
...
#         d = front_scan.ranges[i] / RES
#         # rospy.loginfo(int(point[0]))
#         # rospy.loginfo(int(point[1]))
#         ip, jp, is_hit = bresenham(int((odom.pose.pose.position.x+10)/RES), int((odom.pose.pose.position.y+10)/RES), int((point[0]+10)/RES), int((point[1]+10)/RES), d)
...
#     world = sensor_model_l_prior * np.ones((DIM, DIM), dtype=np.float32)
#     rospy.init_node('SensorModule', anonymous=True)
#     tf_buffer = Buffer()
#     tf_listener = TransformListener(tf_buffer)
#     rospy.loginfo(f"SensorModule node started {DIM} * {DIM}")
